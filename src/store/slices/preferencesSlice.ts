	...
	updatePreferences: async (id, op, data) => {
		...
		try {
			...
			await tryPbOrQueue(
				async () => {
					if (finalOp === "create") {
						if (currentPref) await pb.collection("preferences").create(currentPref);
					} else if (finalOp === "update") {
						if (currentPref) await pb.collection("preferences").update(finalId, currentPref);
					} else if (finalOp === "delete") {
				...
				},
				{
					...
					action: finalOp as "create" | "update" | "delete",
					recordId: finalId,
