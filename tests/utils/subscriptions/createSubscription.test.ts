...
let capturedCallback: ((e: any) => Promise<void>) | null = null;
const mockSubscribe = mock((_pattern: string, cb: any) => {
	capturedCallback = cb;
...
describe("createSubscription", () => {
	...
	test("subscribes to the collection with '*' pattern", () => {
		const mockTable = { get: mock(() => Promise.resolve(null)) } as any;
	...
	test("triggers delta sync when remote record is newer", async () => {
		...
		const mockTable = { get: mock(() => Promise.resolve(localRecord)) } as any;
		createSubscription("datasets", mockTable)();
