...
const mockAdd = mock((_op: any) => Promise.resolve());
...
describe("tryPbOrQueue", () => {
	...
	test("queues offline op when error has status === 0", async () => {
		...
		const addedOp = mockAdd.mock.calls[0][0] as any;
	...
	test("includes a reasonable timestamp in the queued op", async () => {
		...
		const ts = (mockAdd.mock.calls[0][0] as any).timestamp;
	...
	test("does not re-throw any errors (silently swallows)", async () => {
		await expect(
			tryPbOrQueue(mock(() => Promise.reject({ status: 0 })), {
				collection: "datasets" as const,
				action: "create" as const,
				recordId: "x",
				data: null,
		...
		await expect(
			tryPbOrQueue(mock(() => Promise.reject({ status: 500 })), {
				collection: "datasets" as const,
				action: "create" as const,
				recordId: "x",
				data: null,
