...
function createTestStore(initialPrefs: import("@/types/dataset").PreferenceRecord[] = []) {
...
describe("createPreferencesSlice — updatePreferences", () => {
  ...
  test("upsert with existing preference (by ID) → updates", async () => {
    ...
    const pref = store
      .getState()
      .preferences.find((p: import("@/types/dataset").PreferenceRecord) => p.id === "pref1");
  ...
  test("calls tryPbOrQueue with correct action for create", async () => {
    ...
    const offlineOp = mockTryPbOrQueue.mock.calls[0][1] as {
      collection: string;
      action: string;
      data: { value: string };
    };
