...
function makeTableMock() {
	return {
		...
		bulkPut: mock((_items: any[]) => Promise.resolve()),
		toArray: mock((): Promise<any[]> => Promise.resolve([])),
		put: mock((_item: any) => Promise.resolve()),
	};
}
...
const mockDb = {
	...
	transaction: mock(async (...args: any[]) => {
...
describe("localToPbSync", () => {
	...
	test("creates a dataset with its metrics on PocketBase", async () => {
		const datasetRecord = {
			id: "ds1", title: "Weight", description: "", type: "single",
			views: ["line"], created: 1000, updated: 2000,
		};
		const metricRecords = [
	...
	test("creates a measurement with its values on PocketBase", async () => {
		const measurementRecord = {
			id: "meas1", datasetId: "ds1", timestamp: 5000, created: 5000, updated: 5000,
		};
		const valueRecords = [
	...
	test("creates a standalone measurement_value on PocketBase", async () => {
		const data = {
			id: "v1", measurementId: "meas1", metricId: "m1", value: 80,
	...
	test("updates a dataset with specific fields", async () => {
		const datasetRecord = {
			id: "ds1", title: "Updated", description: "new desc", type: "single",
			views: ["bar"], created: 1000, updated: 3000,
		};
	...
	test("processes multiple ops in order", async () => {
		...
		mockOps = [
			{ id: 1, collection: "units", action: "create", recordId: "u1", data: { id: "u1" }, timestamp: 1000 },
			{ id: 2, collection: "datasets", action: "delete", recordId: "ds1", data: null, timestamp: 2000 },
		];
...
describe("pbToLocalSync", () => {
	...
	test("fetches all collections, maps data, and sets state", async () => {
		const pbDatasets = [{ id: "ds1", title: "W", description: "", type: "single", views: [], created: "2024-01-01T00:00:00.000Z", updated: "2024-01-01T00:00:00.000Z" }];
		const pbUnits = [{ id: "u1", name: "Kg", symbol: "kg", created: "2024-01-01T00:00:00.000Z", updated: "2024-01-01T00:00:00.000Z" }];
		const pbMetrics = [{ id: "m1", dataset_id: "ds1", name: "Weight", unit_id: "u1", created: "2024-01-01T00:00:00.000Z", updated: "2024-01-01T00:00:00.000Z" }];
	...
	test("clears and repopulates all Dexie tables", async () => {
		...
		for (const tableName of ["datasets", "metrics", "units", "measurements", "measurement_values", "preferences"] as const) {
...
describe("pbDeltaSync", () => {
	...
	test("uses last_sync_time as the PocketBase filter", async () => {
		const prefs = [{ id: "lst", preference: "last_sync_time", value: "2024-06-15 14:30:00", created: 0, updated: 0 }];
		...
		const dsCall = getMockCollection("datasets").getFullList.mock.calls[0][0];
	...
	test("updates Dexie with changed records", async () => {
		...
		const changedDataset = {
			id: "ds1", title: "Updated", description: "", type: "single",
			views: [], created: "2024-01-01T00:00:00.000Z", updated: "2024-06-15T00:00:00.000Z",
		};
		...
		dbTables.datasets.toArray.mockResolvedValueOnce([{
			id: "ds1", title: "Updated", description: "", type: "single",
			views: [], created: 1704067200000, updated: 1718409600000,
		}]);
	...
	test("rebuilds and sets state from all Dexie data after sync", async () => {
		dbTables.preferences.toArray
			...
			.mockResolvedValueOnce([{ id: "p1", preference: "theme", value: "dark", created: 0, updated: 0 }]);

		const changedUnit = {
			id: "u1", name: "Kg", symbol: "kg",
			created: "2024-01-01T00:00:00.000Z", updated: "2024-06-15T00:00:00.000Z",
		};
		...
		dbTables.datasets.toArray.mockResolvedValueOnce([]);
		dbTables.metrics.toArray.mockResolvedValueOnce([]);
		dbTables.units.toArray.mockResolvedValueOnce([{ id: "u1", name: "Kg", symbol: "kg", created: 0, updated: 0 }]);
		dbTables.measurements.toArray.mockResolvedValueOnce([]);
		dbTables.measurement_values.toArray.mockResolvedValueOnce([]);
	...
	test("updates last_sync_time preference on success", async () => {
		...
		const putArg = dbTables.preferences.put.mock.calls[0][0] as any;
