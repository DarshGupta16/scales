...
- **Dexie.js as Primary UI DB**: The UI components subscribe exclusively to the local Dexie.js database using `useLiveQuery`. Redundant optimistic UI updates via TanStack Query cache have been removed, making the mental model simpler and the data flow unidirectional: Component -> Write to Dexie (Instant UI Update) -> Background sync to Server -> Server updates Dexie -> Instant UI Update.
